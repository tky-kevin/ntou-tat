...
import { useTimetable } from '../../core/api/hooks'
import { useAppStore } from '../../core/store/useAppStore'
import { useLocalDataStore } from '../../core/store/useLocalDataStore'
import { StudentStrip } from '../../components/StudentStrip'
import { CourseSheet } from '../course/CourseSheet'

export function TimetableScreen() {
  const { selectedSemester, timetableViewMode: viewMode } = useAppStore()
  const { data: timetableData, isLoading } = useTimetable(selectedSemester)
  
  const customCoursesMap = useLocalDataStore(s => s.customCourses)
  const deletedCoursesMap = useLocalDataStore(s => s.deletedCourses)
  const setDeletedCourses = useLocalDataStore(s => s.setDeletedCourses)
  
  const customCourses = selectedSemester ? (customCoursesMap[selectedSemester] ?? []) : []
  const deletedCourses = selectedSemester ? (deletedCoursesMap[selectedSemester] ?? []) : []

  const slots = useMemo(() => {
    const fetched = timetableData?.slots ?? []
    const merged = [...fetched, ...customCourses]
    return merged.filter(s => !deletedCourses.includes(s.courseTitle))
  }, [timetableData, customCourses, deletedCourses])

  const [activeCourse, setActiveCourse] = useState<CourseSummary | null>(null)

  const onOpenCourse = (slot: TimetableSlot) => {
    setActiveCourse({
      id: slot.courseId,
  ...
  const handleDeleteCourse = (courseTitle: string) => {
    ...
    setDeletedCourses((prev) => ({
      ...prev,
      [selectedSemester]: [...(prev[selectedSemester] || []), courseTitle]
    }))
